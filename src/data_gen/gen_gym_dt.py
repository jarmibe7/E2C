...
xml_file = mj_path / Path(env_name + ".xml")
xml_text = xml_file.read_text()
xml_text = re.sub(
    r'timestep="[^"]+"',
    f'timestep="{new_dt:.4f}"',
    xml_text
)
# Save modified file
new_xml_filename = f"reacher_timestep_{dt_ms}_ms.xml"
...
def main():
    ...
    # Create env
    env = gym.make(name_to_env[env_name], render_mode="rgb_array", xml_file=new_xml_filename)
    obs, _ = env.reset()
